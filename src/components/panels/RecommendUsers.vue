...
<template>
    <div class="recommended-authors">
        <div class="title"><span>推荐作者</span>
            <a class="page-change"><i class="iconfont ic-search-change"
                                      style="transform: rotate(0deg);"></i>
                换一批
            </a></div>

        <ul class="list">
            <li v-for="user in usersList" :key="user">
                <a :href="user.url" target="_blank" class="avatar">
                    <img :src="user.avatar">
                </a>
                <a class="follow" state="0"><i class="iconfont ic-follow"></i>关注
                </a> <a :href="user.url" target="_blank"
                        class="name">
                {{user.name}}
            </a>
                <p>
